...
namespace cloud_datastore {
  ...
  static constexpr uint64_t MAX_ERROR = 256L;
  ...
  void QuadrupleBuilder::doAvoidDecimal128CollisionsWithDouble() {
    // Collisions may occur when the result matches some double.
    if (this->mantLo == 0LL && (this->mantHi & 0XFFFLL) == 0LL) {
  ...
  int32_t QuadrupleBuilder::parseMantissa(std::vector<uint8_t>& digits,std::array<uint64_t,6>& mantissa) {
    ...
    if (static_cast<int32_t>((digits).size()) - firstDigit > MAX_MANTISSA_LENGTH) {
  ...
  int32_t QuadrupleBuilder::addCarry(std::vector<uint8_t>& digits) {
    for (int32_t i = (static_cast<int32_t>((digits).size())) - 1; i >= (0); i--) { // starting with the lowest digit
      uint8_t c = digits[i];
      if (c == 9) {
        digits[i] = 0;
      } else {
  ...
  void QuadrupleBuilder::findBinaryMantissa(int32_t exp10,double exp2,std::array<uint64_t,6>& mantissa) {
    ...
    bool exact192 = product[4] == 0LL && product[5] >= 0LL && product[5] <= 2LL * MAX_ERROR;
  ...
  void QuadrupleBuilder::multUnpacked6x32byPacked(std::array<uint64_t,6>& factor1,std::array<uint64_t,4>& factor2,std::array<uint64_t,12>& product) {
    ...
    for (int32_t i = (maxFactIdx) - 1; i >= (0); i--) { // compute partial 32-bit products
