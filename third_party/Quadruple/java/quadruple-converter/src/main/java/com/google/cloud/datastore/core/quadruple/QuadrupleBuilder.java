...
public class QuadrupleBuilder {
  ...
  private double findBinaryExponent(int exp10,long[] mantissa) {
    long mant10 = mantissa[0] << 31L | ((mantissa[1]) >>> (1L)); // Higher 63 bits of the mantissa, in range
  ...
  private void divBuffByPower2(long[] buffer,int exp2) {
    ...
    if (exp2 > 0) { // Shift to the right
      ...
      long exp2Shift = ((long)(-exp2));
      buffer[0] = ((buffer[0] << exp2Shift) | ((buffer[1]) >>> (backShift))); // Preserve the high half of buff[0]
      for (int i = (1); i < (maxIdx); i++) {
        buffer[i] = (((buffer[i] << exp2Shift) & LOWER_32_BITS) | ((buffer[i + 1]) >>> (backShift)));
      }
