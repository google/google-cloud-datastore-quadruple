...
export class Quadruple {
  ...
  isZero(): boolean {
    return this.biasedExponent == 0 && this.mantHi == 0n && this.mantLo == 0n;
  }
  ...
  static fromNumber(value: number): Quadruple {
    ...
    let array = new DataView(new ArrayBuffer(8));
    array.setFloat64(0, value);
    let bits = array.getBigUint64(0);
    ...
    if (exponent == 0) {
      ...
      let leadingZeros = QuadrupleBuilder.clz64(mantHi);
  ...
  static fromString(s: string): Quadruple {
    ...
    let digits: number[] = new Array(s.length).fill(0);
    let i = 0;
    let j = 0;
    let exponent = 0;
    let negative = false;
    ...
    let parsed = QuadrupleBuilder.parseDecimal(digits.slice(0, j), exponent);
    return new Quadruple(
      negative,
      parsed.exponent,
      parsed.mantHi,
